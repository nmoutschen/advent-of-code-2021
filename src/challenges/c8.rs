...
fn parse_line(line: &str) -> usize {
    ...
    // Num 1 & 4
    let num_1 = patterns.split_whitespace().filter(|s| s.len() == 2).collect::<Vec<_>>()[0];
    let num_4 = patterns.split_whitespace().filter(|s| s.len() == 4).collect::<Vec<_>>()[0];

    // Segment B
    seg_count.iter().find(|(count, _)| *count == 6).map(|(_, c)| {
        segments.insert(**c, 'b');
    });
    // Segment E
    seg_count.iter().find(|(count, _)| *count == 4).map(|(_, c)| {
        segments.insert(**c, 'e');
    });
    // Segment F
    seg_count.iter().find(|(count, _)| *count == 9).map(|(_, c)| {
        segments.insert(**c, 'f');
    });
    // Segments A & C
    seg_count.iter().filter(|(count, _)| *count == 8).for_each(|(_, c)| {
    ...
    // Segments D & G
    seg_count.iter().filter(|(count, _)| *count == 7).for_each(|(_, c)| {
    ...
    // Remap segments
    let output = output.chars().map(|c| {
        segments.get(&c).unwrap_or(&c).clone()
    }).collect::<String>();
    output.split_whitespace().map(|s| seg_to_num(s)).fold(0, |acc, n| acc * 10 + n)
}
